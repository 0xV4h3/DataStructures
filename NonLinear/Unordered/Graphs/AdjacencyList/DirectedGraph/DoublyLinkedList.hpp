...
public:
    ...
    std::optional<std::reference_wrapper<const std::pair<Key, Value>>> search(const Key& key) const {
        std::shared_lock lock(mtx);
        auto current = head;
        while (current) {
            if (current->data.first == key)
                return std::cref(current->data);
            current = current->next;
        }
        return std::nullopt;
    }
    ...
    template<typename Predicate>
    std::optional<std::reference_wrapper<const std::pair<Key, Value>>> search_if(Predicate pred) const {
        std::shared_lock lock(mtx);
        auto current = head;
        while (current) {
            if (pred(current->data))
